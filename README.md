...
![研数公式速查 2.1 界面](docs/界面预览.png)

## 直接使用

从仓库的 `release/研数公式速查-2.1.exe` 下载后双击运行，不需要安装 Python、Node.js 或数据库。
...
| 快捷键 | 作用 |
|---|---|
| `Ctrl + Shift + Space` | 默认全局唤出或隐藏窗口，可在“快捷键与使用说明”中修改 |
| `Ctrl + K` | 聚焦搜索框 |
| `↑` / `↓` | 在结果中移动 |
| `Enter` | 打开当前结果 |
| `Ctrl + D` | 收藏或取消收藏 |
...
点击界面左下角的“快捷键与使用说明”，再点击快捷键录制框并按下新组合键即可设置。支持 `Ctrl`、`Alt`、`Shift`、`Win` 与字母、数字、空格、`F1–F12` 组合；为避免误触，组合中必须包含 `Ctrl`、`Alt` 或 `Win`。

如果新组合键已被其他软件占用，程序会保留原快捷键；启动时若已保存的组合键失效，会尝试使用安全备用键 `Ctrl + Alt + M`。也可以从系统托盘唤出程序。
...
- 实际 EXE 启动、中文显示、搜索、独立滚动、窄窗口布局和全局快捷键检查
